...
from .preprocessor import TrainPreProcessor, QueryPreProcessor, CorpusPreProcessor, RankPreProcessor
from ..arguments import DataArguments
import os

DEFAULT_PROCESSORS = [TrainPreProcessor, QueryPreProcessor, CorpusPreProcessor, RankPreProcessor]
PROCESSOR_INFO = {
    ...
    def process(self, shard_num=1, shard_idx=0):
        ...
        if self.preprocessor is not None:
            self.dataset = self.dataset.map(
                ...
                cache_file_name=os.path.join(self.cache_dir, 'msmarco-passage-train.cache'),
    ...
    def process(self, shard_num=1, shard_idx=0):
        ...
        if self.preprocessor is not None:
            self.dataset = self.dataset.map(
                ...
                remove_columns=self.dataset.column_names,
                load_from_cache_file=True,
                cache_file_name=os.path.join(self.cache_dir, 'msmarco-passage-dev.cache'),
    ...
    def process(self, shard_num=1, shard_idx=0):
        ...
        if self.preprocessor is not None:
            self.dataset = self.dataset.map(
                ...
                remove_columns=self.dataset.column_names,
                load_from_cache_file=True,
                cache_file_name=os.path.join(self.cache_dir, f'msmarco-passage-corpus-{shard_idx}.cache'),
                desc="Running tokenization",
            )
        return self.dataset
    ...
    def process(self, shard_num=1, shard_idx=0):
        ...
        if self.preprocessor is not None:
            self.dataset = self.dataset.map(
                ...
                remove_columns=self.dataset.column_names,
                load_from_cache_file=True,
                cache_file_name=os.path.join(self.cache_dir, 'msmarco-passage-rank.cache'),
                desc="Running tokenizer on train dataset",
            )
        return self.dataset
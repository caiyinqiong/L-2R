# 支持： 从[1, threshold_rank]中score<threshold_score的集合 + (threshold_rank, top]中采样出sample个样本
import json
...
class BasicHardNegativeMiner:
    def __init__(self, results_path, rank_result_path, corpus_dataset, sample, top, threshold_rank, threshold_score):
        self.corpus_data = corpus_dataset
        self.sample=sample

        rank_results = self._read_rank_result(rank_result_path)
        self.retrieval_results, self.top_results = self._read_result(rank_results, results_path, top, threshold_rank, threshold_score)
    ...
    @staticmethod
    def _read_result(rank_results, results_path, top, threshold_rank, threshold_score):
        retrieval_results = {}
        top_results = {}
    ...
    def __call__(self, example):
        ...
        if query_id in self.retrieval_results:
            ...
            if query_id in self.top_results:
                retrieved_docid.extend(self.top_results[query_id])
...
if __name__ == '__main__':
    ...
    if args.em:
        ...
        miner = BasicHardNegativeMiner(args.result_path, args.rank_result_path, corpus_data, args.sample, args.top, args.threshold_rank, args.threshold_score)

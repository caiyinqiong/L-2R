...
# query方向
class OurUpdateEmbHorizontal(object):
    ...
    def replace(self, buffer):
        ...
        for qid, candidate_docids in tqdm(self.candidate_neg_docids.items(), total=len(self.candidate_neg_docids)):
            ...
            if place_left == 0:  # buffer满， 执行替换
                ...
                q_reps, p_reps = self.get_model_out(buffer, model_temp, qid, mem_eval_docids_lst+upsample_mem_docids_lst+upsample_candidate_docids_lst)  # [1, 768]; [mem_eval_size+mem_upsample_num+new_upsample_num, 768]
                # p在q方向上的投影向量
                ...
                mem_eval_p_q = p_q[:mem_eval_size,:]  # [mem_eval_size, 768]
                mem_p_q = p_q[mem_eval_size:mem_eval_size+mem_upsample_num,:]  # [mem_upsample_num, 768]
                new_p_q = p_q[mem_eval_size+mem_upsample_num:,:]  # [new_upsample_num, 768]

                memeval_p_q = mem_eval_p_q.unsqueeze(dim=0).repeat(mem_upsample_num,1,1)  # [mem_upsample_num, mem_eval_size, 768]
                mem_p_q = mem_p_q.unsqueeze(dim=1).repeat(1,mem_eval_size,1)  # [mem_upsample_num, mem_eval_size, 768]
                mem_inter_dis = F.pairwise_distance(mem_p_q.reshape(-1, mem_p_q.size(-1)), memeval_p_q.reshape(-1, memeval_p_q.size(-1)), p=2.0)  # [mem_upsample_num*mem_eval_size]
                mem_inter_dis = mem_inter_dis.reshape(mem_p_q.size(0), mem_p_q.size(1))  # [mem_upsample_num, mem_eval_size]
                ...
                neweval_p_q = mem_eval_p_q.unsqueeze(dim=0).repeat(new_upsample_num,1,1)  # [new_upsample_num, mem_eval_size, 768]
                new_p_q = new_p_q.unsqueeze(dim=1).repeat(1,mem_eval_size,1)  # [new_upsample_num, mem_eval_size, 768]
                new_inter_dis = F.pairwise_distance(new_p_q.reshape(-1, new_p_q.size(-1)), neweval_p_q.reshape(-1, neweval_p_q.size(-1)), p=2.0)  # [new_upsample_num*mem_eval_size]
                new_inter_dis = new_inter_dis.reshape(new_p_q.size(0), new_p_q.size(1))  # [mem_upsample_num, mem_eval_size]
                ...
                mem_p_q = p_q[:mem_eval_size,:]  # [mem_eval_size, 768]
                candidate_p_q = p_q[mem_eval_size:,:]  # [upsample_num, 768]

                mem_p_q = mem_p_q.unsqueeze(dim=0).repeat(upsample_num,1,1)  # [upsample_num, mem_eval_size, 768]
                candidate_p_q = candidate_p_q.unsqueeze(dim=1).repeat(1,mem_eval_size,1)  # [upsample_num, mem_eval_size, 768]
                inter_dis = F.pairwise_distance(candidate_p_q.reshape(-1, candidate_p_q.size(-1)), mem_p_q.reshape(-1, mem_p_q.size(-1)), p=2.0)  # [upsample_num*mem_eval_size]
                inter_dis = inter_dis.reshape(candidate_p_q.size(0), candidate_p_q.size(1))

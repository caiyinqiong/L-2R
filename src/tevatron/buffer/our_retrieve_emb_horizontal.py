...
# query方向
class Our_retrieve_emb_horizontal(object):
    ...
    def get_mem_data(self, new_model_out, index_new, mem_model_out, mem_bz):
        ...
        all_p_reps = torch.cat((p_reps, choiced_new_reps), dim=1)  # [8, (n-1)+new_bz, 768]
        ...
        mem_p_q = all_p_q[:,:p_reps.size(1),:]  # [8, n-1, 768]
        new_p_q = all_p_q[:,p_reps.size(1):,:]  # [8, new_bz, 768]

        mem_p_q = mem_p_q.unsqueeze(dim=2).repeat(1,1,new_p_q.size(1),1)  # [8, n-1, new_bz, 768]
        new_p_q = new_p_q.unsqueeze(dim=1).repeat(1,mem_p_q.size(1),1,1)  # [8, n-1, new_bz, 768]
        inter_dis = F.pairwise_distance(mem_p_q.reshape(-1, mem_p_q.size(-1)), new_p_q.reshape(-1, new_p_q.size(-1)), p=2.0) # [8*(n-1)*new_bz]
        inter_dis = inter_dis.reshape(mem_p_q.size(0), mem_p_q.size(1), mem_p_q.size(2))  # [8, n-1, new_bz]
    ...
    def get_new_data(self, new_model_out, new_bz, alpha, beta):
        ...
        neg_p_q_1 = neg_p_q.unsqueeze(dim=2).repeat(1,1,neg_p_q.size(1),1)  # [8, n-1, n-1, 768]
        neg_p_q_2 = neg_p_q.unsqueeze(dim=1).repeat(1,neg_p_q.size(1),1,1)  # [8, n-1, n-1, 768]
        inter_dis = F.pairwise_distance(neg_p_q_1.reshape(-1, neg_p_q_1.size(-1)), neg_p_q_2.reshape(-1, neg_p_q_2.size(-1)), p=2.0) # [8*(n-1)*(n-1)]
        inter_dis = inter_dis.reshape(neg_p_q_1.size(0), neg_p_q_1.size(1), neg_p_q_1.size(2))  # [8, n-1, n-1]

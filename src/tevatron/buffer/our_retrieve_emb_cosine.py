...
class Our_retrieve_emb_cosine(object):
    ...
    def retrieve(self, buffer, qid_lst, docids_lst, **kwargs):
        ...
        if self.params.compatible:   # 正例用 old embedding
            ...
            doc_oldemb = torch.tensor(np.array(doc_oldemb)).to(self.train_params.device)  # [num_q, 768]
        ...
        if mem_upsample > 0 and mem_bz > 0:
            ...
            if self.params.compatible:   # 全部用old embedding，不需要再过模型
                ...
                for docid in mem_upsample_docids_lst:
                    doc_oldemb.append(buffer.buffer_did2emb[int(docid)])
                doc_oldemb = torch.tensor(np.array(doc_oldemb)).to(self.train_params.device) #.to('cuda:1')  # [num_q*(1+mem_upsample), 768]
                index_mem = self.get_mem_data(new_model_out, index_new, doc_oldemb, mem_bz)  # [batch_size, mem_bz]
            else:
                ...
                index_mem = self.get_mem_data(new_model_out, index_new, mem_p_reps, mem_bz)  # [batch_size, mem_bz]
        ...
        if self.params.compatible:
            doc_oldemb = doc_oldemb.reshape(batch_size, -1, doc_oldemb.size(-1))  # # [batch_size, 1+ mem_upsample, 768]
            doc_oldemb = torch.cat((doc_oldemb[:,:1,:], torch.gather(doc_oldemb, 1, index_mem.unsqueeze(dim=2).repeat(1,1,doc_oldemb.size(-1)))), dim=1) # # [batch_size, 1+ mem_bz, 768]
            doc_oldemb = doc_oldemb.reshape(-1, doc_oldemb.size(-1))  # [batch_size*(1+mem_bz), 768]

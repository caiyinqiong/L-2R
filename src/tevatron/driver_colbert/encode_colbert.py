...
def main():
        ...
        with torch.cuda.amp.autocast() if training_args.fp16 else nullcontext():
            with torch.no_grad():
                ...
                if data_args.encode_is_qry:
                    ...
                    for index, (emb, lenn) in enumerate(zip(model_output.q_reps, model_output.scores)):
                        encoded.append(emb.cpu().detach().numpy())
                        lookup_indices.extend([batch_ids[index]] * int(lenn.cpu().detach()))
                else:
                    model_output: EncoderOutput = model(passage=batch)
                    for index, (emb, lenn) in enumerate(zip(model_output.p_reps, model_output.scores)):
                        encoded.append(emb.cpu().detach().numpy())
                        lookup_indices.extend([batch_ids[index]] * int(lenn.cpu().detach()))
    ...
    encode_memmap = np.memmap(data_args.encoded_save_path+'.encode', 
        dtype=np.float32, mode="w+", shape=(len(encoded), dim)) 
    id_memmap = np.memmap(data_args.encoded_save_path+'.id', 
        dtype=np.int32, mode="w+", shape=(len(lookup_indices), ))
    encode_memmap[:] = encoded
    id_memmap[:] = np.array(lookup_indices)
